...
export function validateExport(data: unknown): GanttExport | null {
  ...
  if (
    !isDateStr(tc.startDate) ||
    !isDateStr(tc.endDate) ||
    typeof tc.zoomLevel !== "number"
  )
    return null;
  if (
    tc.viewMode !== undefined &&
    tc.viewMode !== "day" &&
    tc.viewMode !== "week" &&
    tc.viewMode !== "month" &&
    tc.viewMode !== "year"
  ...
  if (
    tc.customDateRange !== undefined &&
    typeof tc.customDateRange !== "boolean"
  )
    return null;
