...
function computeDropTarget(
  ...
  if (!dragSubtaskId) {
    ...
    for (let i = 0; i <= groups.length; i++) {
      const boundaryY =
        i < groups.length ? groups[i].startY : groups[groups.length - 1].endY;
    ...
    if (bestBoundaryIdx === draggedIdx || bestBoundaryIdx === draggedIdx + 1)
      return null;

    const toIndex =
      bestBoundaryIdx > draggedIdx ? bestBoundaryIdx - 1 : bestBoundaryIdx;
    const indicatorY =
      bestBoundaryIdx < groups.length
    ...
    if (targetTaskId === dragTaskId) {
      const origIdx = sourceTask.subtasks.findIndex(
        (s) => s.id === dragSubtaskId,
      );
      if (targetSubIndex === origIdx || targetSubIndex === origIdx + 1)
        return null;
    }
    ...
    return {
      ...
      apply: (prev) => {
        const sub = prev
          .find((t) => t.id === dragTaskId)
          ?.subtasks.find((s) => s.id === dragSubtaskId);
        if (!sub) return prev;
        ...
        let result = prev.map((t) => {
          if (t.id === dragTaskId) {
            return {
              ...t,
              subtasks: t.subtasks.filter((s) => s.id !== dragSubtaskId),
            };
          }
          return t;
        });
          ...
          if (dragInfo.subtaskId) {
            return {
              ...
              subtasks: t.subtasks.map((s) =>
                s.id === dragInfo.subtaskId
                  ? { ...s, startDate: fmt(newStart), endDate: fmt(newEnd) }
                  : s,
              ),
            };
          }
