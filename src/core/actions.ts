...
export const undoAtom = atom(null, (get, set) => {
  ...
  set(redoStackAtom, [
    ...get(redoStackAtom),
    { tasks: get(tasksAtom), markers: get(markersAtom) },
  ]);
...
export const updateTaskAtom = atom(null, (get, set, updated: Task) => {
  ...
  set(
    tasksAtom,
    get(tasksAtom).map((t) => (t.id === updated.id ? updated : t)),
  );
});
...
export const deleteTaskAtom = atom(null, (get, set, taskId: string) => {
  ...
  set(
    tasksAtom,
    get(tasksAtom).filter((t) => t.id !== taskId),
  );
});
...
export const addSubtaskAtom = atom(
  ...
  (
    get,
    set,
    { taskId, subtask }: { taskId: string; subtask: Omit<Subtask, "id"> },
  ) => {
    set(pushUndoAtom);
    set(
      tasksAtom,
      get(tasksAtom).map((t) =>
        t.id === taskId
          ? {
              ...
              subtasks: [
                ...t.subtasks,
                { ...subtask, id: "s-" + uid(), color: t.color },
              ],
            }
          : t,
      ),
...
export const updateMarkerAtom = atom(null, (get, set, updated: Marker) => {
  set(pushUndoAtom);
  set(
    markersAtom,
    get(markersAtom).map((m) => (m.id === updated.id ? updated : m)),
  );
});
