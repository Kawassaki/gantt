...
export function GanttToolbar({
  ...
  return (
    <div className={`flex items-center gap-1.5 ${className}`}>
      {title && (
        <span className={`font-bold mr-auto ${titleClass}`}>{title}</span>
      )}

      <div className="flex items-center gap-1">
        <button
          className={`${btnBase} earth-toolbar-btn--zoom-out`}
          onClick={() => setZoom(config.zoomLevel - 10)}
          title="Zoom out"
        >
          <ZoomOut size={14} />
        </button>
        <span className="text-[10px] opacity-50 tabular-nums w-7 text-center select-none">
          {config.zoomLevel}
        </span>
        <button
          className={`${btnBase} earth-toolbar-btn--zoom-in`}
          onClick={() => setZoom(config.zoomLevel + 10)}
          title="Zoom in"
        >
          <ZoomIn size={14} />
      ...
      <div className="flex items-center gap-1">
        <button
          className={`${btnBase} earth-toolbar-btn--undo`}
          onClick={() => undo()}
        ...
        <button
          className={`${btnBase} earth-toolbar-btn--redo`}
          onClick={() => redo()}
      ...
      <div className="flex items-center gap-1">
        <button
          className={`${btnBase} earth-toolbar-btn--export earth-toolbar-btn--io`}
        ...
        <button
          className={`${btnBase} earth-toolbar-btn--import earth-toolbar-btn--io`}
